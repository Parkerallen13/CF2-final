...
export default function Favorites() {
  ...
  useEffect(() => {
    ...
    setFavoriteRecipes(favorites); // Filter recipes based on saved IDs
  }, []);

  // Handle toggle to save or unsave a recipe
  const handleToggleSave = (id: string) => {
    const updatedSavedIds = savedIds.includes(id)
      ? savedIds.filter((savedId) => savedId !== id) // Remove from saved
      : [...savedIds, id]; // Add to saved
